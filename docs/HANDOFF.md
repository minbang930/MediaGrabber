...
1. Build/load `diag/mse-drm-boundary`, reload the page, and play for several seconds.
2. Open the MSE candidate; no download attempt is required.
3. Report the full bracketed label, especially `eme=`, `initData=`, and each buffer's `drm=` / `codec=` fields.
