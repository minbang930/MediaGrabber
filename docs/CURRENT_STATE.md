...
- Closed PR #9 classified the playlist as HLS v6 media with 1118 `other`-extension segments, AES-128/identity encryption, no init map, no byte ranges, no LL-HLS parts, and no I-frame-only mode.
- Branch `diag/hls-browser-responses` compares only coarse browser-observed response metadata for known segment/key URLs to determine whether browser and FFmpeg request context diverge.
