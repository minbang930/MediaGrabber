...
- Draft PR #23 tests 8× playback only during an explicit capture. First validation showed capture remained healthy but no rate state appeared, meaning the hook did not select a media element rather than the player rejecting 8×. The experiment now keeps exact blob matching as preferred and adds a fallback only when the capture frame has exactly one currently playing video/media element after the first captured fragment.
