...
- Downloading the remaining HLS candidate still fails with the generic FFmpeg "could not open stream" error. Branch `diag/hls-ffmpeg-open` adds non-sensitive HLS structure/rewrite/error classification without exposing media URLs or request credentials.
