...
1. Build/load `diag/mse-resource-correlation`, reload the page, and play for several seconds.
2. Open the MSE candidate; no download attempt is required.
3. Report the full bracketed MSE diagnostic label for both buffers, especially `box=`, `r`, `x`, `n`, and `via=` fields.
