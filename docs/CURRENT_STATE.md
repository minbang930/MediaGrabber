...
3. Removing the MSE injector is not a viable final fix. Diagnostics traced the 14 popup entries to 12 extensionless HTTPS `<source>` descendants, 1 `blob:` currentSrc, and 1 HTTPS HLS response. PR #6 merged as `2bfc0884d8380fd31f9ed97310da3de39f5f0b45`; user manual validation confirmed the popup dropped from 14 entries to 1 while playback remained normal.
...
- Draft PR #8 (`fix/hls-extensionless-segments`) removes the prior terminal `invalid-data` failure under manual testing, but FFmpeg then reports that the output contains no stream. Playback remains normal.
- Branch `diag/hls-no-stream` adds manifest-structure classification to determine whether the selected HLS is ordinary A/V, fMP4, subtitle/image, LL-HLS, or protected media before another fix is attempted.
