...
class MediaDetector {
  ...
  private mseCaptureDiagnostics: Array<{
    id: number;
    mime: string;
    appends: number;
  ...
  private setupMSEListener(): void {
    window.addEventListener('message', (event) => {
      ...
      switch (msg.type) {
        ...
        case 'capture-diagnostic':
          if (Array.isArray(msg.buffers)) {
            this.mseCaptureDiagnostics = msg.buffers;
            this.sendMSEToBackground();
          }
          break;
  ...
  private sendMSEToBackground(): void {
    ...
    const diagnosticSummary = this.mseCaptureDiagnostics
      .map((buffer) => {
        const boxes = Object.entries(buffer.boxes || {})
          ...
          .join(',');
        const initiators = Object.entries(buffer.initiators || {})
          .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))
          .slice(0, 2)
          .map(([name, count]) => `${name}:${count}`)
          .join(',');
    ...
    const qualities: VideoQuality[] = [{
      ...
      url,
      bitrate: 0,
      label: diagnosticSummary ? `${baseLabel} [${diagnosticSummary}]` : baseLabel,
